...
from utils.storage_local import save_to_local_storage
            ...
            if "videos" in result:
                for video in result["videos"][:5]:  # Get top 5 videos
                    ...
                    if video_id:
                        ...
                        if thumbnail_base64:
                            # Save thumbnail locally instead of Cloudinary
                            ...
                            if local_img_url:
                                local_img_url = f"http://localhost:8000{local_img_url}"
...
# Alternative: Simplified version without video thumbnails
def prepare_resources_simple(highlight_mapping: dict):
    """
    Simplified version that doesn't download video thumbnails
    """
    resources = {}

    def search_both_types_simple(sentence):
        """Search without thumbnail processing"""
        articles = serper_search(sentence, "search")
        videos = serper_search(sentence, "videos")
        
        # For videos, just use YouTube's default thumbnail URL
        for video in videos:
            if video.get("link"):
                video_id = extract_youtube_video_id(video["link"])
                if video_id:
                    video["image_url"] = f"https://img.youtube.com/vi/{video_id}/mqdefault.jpg"
    ...
    with ThreadPoolExecutor() as executor:
        future_to_index = {
            executor.submit(search_both_types_simple, sentence): index
            for index, sentence in highlight_mapping.items()
        }
